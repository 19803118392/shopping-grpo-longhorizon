...
当前正式数据契约固定为 Environment v2.1 / Reward v3。用于 SFT 的 accepted 轨迹必须无错误、执行购买、得到环境终局结果，并满足 `reward_detail.reward_version == "shopsimulator-reward-v3"`、`reward_type == "gold_purchase"`、`reward_valid == true` 和 `purchase_success == true`。Reward v3 的 `partial_alternative_purchase`、`wrong_purchase`、`repeat_loop`、`max_steps` 和 `reward_unverifiable` 轨迹不得作为 gold SFT 目标。此外，每个工具调用必须是 JSON 对象、不得含 tool schema 未声明字段、与映射后的环境动作一致，并且来自紧邻的 observation。
